    ...
    IdentifierReturn evaluateFunction(const std::string &name,
      ...
      if (name == "ASCII")
        if (!args[0].isstring || args[0].s.empty()) {
          ...
          temp.d =
              static_cast<double>(static_cast<unsigned char>(args[0].s[0]));
          return temp;
        }
      ...
      if (name == "SIN" || name == "COS" || name == "TAN" || name == "SQR" ||
              ...
              name == "CLOG" || name == "EXP" || name == "INT" ||
              name == "ROUND" || name == "FLOOR" ||
              name == "CEIL" | name == "RND" || name = "DET")
        if (args[0].isstring) {
      ...
      if (name == "LOGX" || name == "POW")
        if (args[1].isstring) {
          std::cerr << "String passed [...]  " << name << "(" << args[0].d
                    << ",[" << args[1].s << "])  line:" << args[0].linenumber
                    << std::endl;
          temp.d = 0.0;
          return temp;
        }
          ...
          if (peek() == '(') {
            ...
            if (peek() != ')') {
              ...
              do {
                ...
                args.push_back(
                    makeArgsInfo(linenumber, name, false, "", argval));
              } while (peek() == ',' && get());
            }
            if (get() != ')')
