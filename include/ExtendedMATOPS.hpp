...
Matrix multiply(const Matrix& A, const Matrix& B) {
    ...
    if (A.empty() || p != A[0].size()) throw std::invalid_argument("Dimension mismatch");
...
Matrix power(const Matrix& A, int exp) {
    if (A.empty() || A.size() != A[0].size()) throw std::invalid_argument("Power requires square matrix");
    size_t n = A.size();
...
double trace(const Matrix& A) {
    if (A.empty() || A.size() != A[0].size()) throw std::invalid_argument("Trace requires square matrix");
    double t = 0;
    for (size_t i = 0; i < A.size(); ++i) t += A[i][i];
    return t;
}
...
int rank(Matrix A) {
    ...
    for (int c = 0; c < m && r < n; ++c) {
        int pivot = r;
        for (int i = r; i < n; ++i) if (std::fabs(A[i][c]) > std::fabs(A[pivot][c])) pivot = i;
        if (std::fabs(A[pivot][c]) < EPS) continue;
...
Matrix solve(const Matrix& A, const Matrix& B) {
    size_t n = A.size();
    if (n == 0 || A[0].size() != n || B.size() != n) throw std::invalid_argument("Dimension mismatch for solve");
    // Form augmented
    Matrix aug(n, std::vector<double>(n + B[0].size()));
    for (size_t i = 0; i < n; ++i) {
        for (size_t j = 0; j < n; ++j) aug[i][j] = A[i][j];
        for (size_t j = 0; j < B[0].size(); ++j) aug[i][n+j] = B[i][j];
    }
    // Forward eliminate
    for (size_t i = 0; i < n; ++i) {
        size_t pivot = i;
        for (size_t r = i; r < n; ++r) if (std::fabs(aug[r][i]) > std::fabs(aug[pivot][i])) pivot = r;
        if (std::fabs(aug[pivot][i]) < 1e-9) throw std::runtime_error("Singular matrix");
        std::swap(aug[i], aug[pivot]);
        double d = aug[i][i];
        for (size_t j = i; j < aug[i].size(); ++j) aug[i][j] /= d;
        for (size_t r = 0; r < n; ++r) if (r != i) {
            double f = aug[r][i];
            for (size_t j = i; j < aug[r].size(); ++j) aug[r][j] -= f * aug[i][j];
        }
    }
    // Extract solution
...
std::pair<Matrix,Matrix> lu(const Matrix& A) {
    size_t n = A.size(); if(n==0||A[0].size()!=n) throw std::invalid_argument("LU requires square");
    Matrix L = identity(n), U = zeros(n,n);
    for(size_t i=0;i<n;++i) {
        for(size_t k=i; k<n; ++k) {
            double sum=0; for(size_t j=0;j<i;++j) sum+=L[i][j]*U[j][k];
            U[i][k]=A[i][k]-sum;
        }
        for(size_t k=i+1; k<n; ++k) {
            double sum=0; for(size_t j=0;j<i;++j) sum+=L[k][j]*U[j][i];
...
std::pair<Matrix,Matrix> qr(const Matrix& A) {
    ...
    for(size_t k=0; k<n; ++k) {
        // copy column
        for(size_t i=0;i<m;++i) Q[i][k] = A[i][k];
        for(size_t j=0;j<k;++j) {
            double dot=0; for(size_t i=0;i<m;++i) dot+=Q[i][j]*A[i][k];
            R[j][k]=dot;
            for(size_t i=0;i<m;++i) Q[i][k] -= dot*Q[i][j];
        }
        double norm=0; for(size_t i=0;i<m;++i) norm+=Q[i][k]*Q[i][k]; norm=std::sqrt(norm);
        R[k][k]=norm;
        if(norm<1e-9) throw std::runtime_error("Rank-deficient");
        for(size_t i=0;i<m;++i) Q[i][k]/=norm;
    }
    return {Q,R};
}

// SVD and Eigen: Not implemented
std::tuple<Matrix,Matrix,Matrix> svd(const Matrix&){ throw std::runtime_error("SVD not implemented"); }
std::pair<Matrix,std::vector<double>> eig(const Matrix&){ throw std::runtime_error("Eigen not implemented"); }

// Element-wise functions
Matrix exp(const Matrix& A){ Matrix B=A; for(auto& row:B) for(auto& v:row) v=std::exp(v); return B; }
Matrix sin(const Matrix& A){ Matrix B=A; for(auto& row:B) for(auto& v:row) v=std::sin(v); return B; }

// Concatenation
Matrix hconcat(const Matrix& A, const Matrix& B) {
    if(A.size()!=B.size()) throw std::invalid_argument("Row count mismatch");
    Matrix C=A;
    for(size_t i=0;i<C.size();++i) C[i].insert(C[i].end(), B[i].begin(), B[i].end());
    return C;
}
Matrix vconcat(const Matrix& A, const Matrix& B) {
    if(A[0].size()!=B[0].size()) throw std::invalid_argument("Column count mismatch");
    Matrix C=A;
