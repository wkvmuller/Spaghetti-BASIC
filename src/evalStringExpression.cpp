...
// Evaluates a string expression, supporting variables, literals, and string functions.
std::string evalStringExpression(const std::string &expr) {
    ...
    if (pos < expr.size() && expr[pos] == '"') {
        ...
        if (pos >= expr.size()) throw std::runtime_error("Unterminated string literal");
    ...
    if (pos < expr.size() && std::isalpha(expr[pos])) {
        size_t start = pos;
        while (pos < expr.size() && (std::isalnum(expr[pos]) || expr[pos]=='_' || expr[pos]=='$')) ++pos;
        ...
        if (pos < expr.size() && expr[pos] == '(') {
            ...
            if (id == "STRING$") {
                ...
                std::string fill = args.size() > 1 ? evalStringExpression(args[1]) : " ";
        ...
        if (!id.empty() && id.back() == '$') {
            ...
            if (it != program.stringVariables.end()) return it->second;
            return "";
        }
    }
