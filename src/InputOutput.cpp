...
void printMatrix(const std::string &name, int rows, int cols,
    ...
    for (int i = 0; i < rows; ++i) {
        for (int j = 0; j < cols; ++j) {
            double value = 0.0;
            if (dense) {
                // removed obsolete dense matrix bounds check
                    // removed obsolete matrix indexing
            } else if (sparse) {
 ...
 * void executeMATPRINT(const std::string &line, std::ostream &out = std::cout) {
  ...
  while (std::getline(ss, name, ',')) {
    ...
    auto itNum = program.matrices.find(name);
    if (itNum != program.matrices.end()) {
