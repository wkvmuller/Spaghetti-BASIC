...
std::string evalStringExpression(const std::string &expr) {
    size_t pos = 0;
    auto skipWS = [&]() {
        while (pos < expr.size() && std::isspace((unsigned char)expr[pos])) ++pos;
    };
    skipWS();
    // 1) String literal
    if (pos < expr.size() && expr[pos] == '"') {
        ...
        skipWS();
        if (pos != expr.size())
            throw std::runtime_error("SYNTAX ERROR: Trailing characters after string literal");
        return lit;
    }
    ...
    if (pos < expr.size() && std::isalpha((unsigned char)expr[pos])) {
        ...
        size_t start = pos;
        while (pos < expr.size() &&
              (std::isalnum((unsigned char)expr[pos]) || expr[pos]=='_' || expr[pos]=='$'))
            ++pos;
        ...
        if (pos < expr.size() && expr[pos] == '(') {
            ...
            args.push_back(trim(expr.substr(argStart, pos - argStart)));
            if (pos >= expr.size() || expr[pos] != ')')
                throw std::runtime_error("SYNTAX ERROR: Missing ')' in string function call");
            ++pos; skipWS();
            if (pos != expr.size())
                throw std::runtime_error("SYNTAX ERROR: Trailing characters after function call");

            // Dispatch known functions
            if (id == "LEFT$") {
                if (args.size() != 2) throw std::runtime_error("SYNTAX ERROR: LEFT$ requires 2 args");
                std::string s = evalStringExpression(args[0]);
                int n = static_cast<int>(std::stoi(args[1]));
                return s.substr(0, n);
            }
            if (id == "RIGHT$") {
                if (args.size() != 2) throw std::runtime_error("SYNTAX ERROR: RIGHT$ requires 2 args");
                std::string s = evalStringExpression(args[0]);
                int n = static_cast<int>(std::stoi(args[1]));
            ...
            if (id == "MID$") {
                if (args.size() != 3) throw std::runtime_error("SYNTAX ERROR: MID$ requires 3 args");
                std::string s = evalStringExpression(args[0]);
            ...
            if (id == "LEN$") {
                if (args.size() != 1) throw std::runtime_error("SYNTAX ERROR: LEN$ requires 1 arg");
                std::string s = evalStringExpression(args[0]);
                return std::to_string(s.length());
            }
            if (id == "CHR$") {
                if (args.size() != 1) throw std::runtime_error("SYNTAX ERROR: CHR$ requires 1 arg");
                int code = std::stoi(args[0]);
                return std::string(1, static_cast<char>(code));
            }
            if (id == "STRING$") {
                if (args.size() < 1 || args.size() > 2)
                    throw std::runtime_error("SYNTAX ERROR: STRING$ requires 1 or 2 args");
            ...
            if (id == "TIME$") {
                if (!args.empty()) throw std::runtime_error("SYNTAX ERROR: TIME$ takes no args");
            ...
            if (id == "DATE$") {
                if (!args.empty()) throw std::runtime_error("SYNTAX ERROR: DATE$ takes no args");
                std::time_t t = std::time(nullptr);
                std::tm tm;
                localtime_r(&t,&tm);
                char buf[11];
        ...
        if (!id.empty() && id.back() == '$') {
            ...
            if (it != program.stringVariables.end()) return it->second.stringValue;
...
double evalExpression(const std::string &expr) {
    ...
    parseFactor = [&]() -> double {
        ...
        if (pos<expr.size() && expr[pos]=='(') {
        ...
        else if (pos<expr.size() && (std::isdigit((unsigned char)expr[pos])||expr[pos]=='.')) {
            size_t start = pos;
            ...
            if (pos<expr.size()&&(expr[pos]=='e'||expr[pos]=='E')) {
                ...
                while (pos<expr.size()&&std::isdigit((unsigned char)expr[pos])) ++pos;
            }
            try {
                value = std::stod(expr.substr(start,pos-start));
            } catch (...) {
                throw std::runtime_error("Invalid number: " + expr.substr(start,pos-start));
            }
        }
        else {
            value = parsePrimary();
    ...
    parsePrimary = [&]() -> double {
        skipWS();
        if (pos>=expr.size() || !std::isalpha((unsigned char)expr[pos]))
            throw std::runtime_error(std::string("Unexpected character '") + (pos<expr.size()?expr[pos]:'?') + "' in expression");
        // read identifier
        size_t start = pos;
        while (pos<expr.size() && (std::isalnum((unsigned char)expr[pos])||expr[pos]=='_')) ++pos;
        std::string id = expr.substr(start,pos-start);
        std::string idUp = id;
        for (auto &c:idUp) c = std::toupper((unsigned char)c);
...
while ((p = body.find(param, p)) != std::string::npos) {
    ...
    p += argStr.size();
            ...
            if (idUp=="SIN")   return std::sin(arg);
            if (idUp=="COS")   return std::cos(arg);
            if (idUp=="TAN")   return std::tan(arg);
            if (idUp=="ATN")   return std::atan(arg);
            if (idUp=="EXP")   return std::exp(arg);
            if (idUp=="LOG10") return std::log10(arg);
            if (idUp=="CLOG")  return std::log(arg);
            if (idUp=="SQR")   return std::sqrt(arg);
            if (idUp=="INT")   return std::floor(arg);
            if (idUp=="RND")   return std::rand()/(double)RAND_MAX;
            if (idUp=="DEG2RAD") return arg * 3.14159265358979323846 / 180.0;
            if (idUp=="RAD2DEG") return arg * 180.0 / 3.14159265358979323846;
